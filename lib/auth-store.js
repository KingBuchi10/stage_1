    ...
    const authRequestSchema = new mongoose.Schema(
      {
        ...
        requested_login: { type: String, default: "" },
...
class MemoryAuthStore {
  constructor() {
    ...
    this.authRequestIdsByState = new Map();
  ...
  async createAuthRequest(payload) {
    ...
    this.authRequestIdsByState.set(record.state, record.id);
    return record;
  }
  ...
  async findAuthRequestByState(state) {
    const id = this.authRequestIdsByState.get(state);

    if (!id) {
      return null;
    }

    return this.findAuthRequestById(id);
  }
  ...
  async consumeAuthRequestByState(state) {
    const record = await this.findAuthRequestByState(state);

    if (!record) {
      return null;
    }

    return this.consumeAuthRequest(record.id);
  }
