...
export function buildGithubAuthorizationUrl({ requestId }) {
  const config = assertGithubConfig();
...
export async function startGithubOAuth({
  ...
  return {
    authorizationUrl: buildGithubAuthorizationUrl({ requestId: authRequest.id }),
    request: authRequest,
  };
}

export async function completeGithubOAuth({ requestId, githubCode }) {
  if (typeof githubCode !== "string" || githubCode.trim() === "") {
    throw createAppError(400, "Missing code");
  }

  const store = await getAuthStore();
  const authRequest = await store.findAuthRequestById(requestId);
  ...
  const role = resolveRoleForGithubUser({
