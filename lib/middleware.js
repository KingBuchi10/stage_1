...
export function validateApiVersion(options = {}) {
  return (req, res, next) => {
    // Only validate version for v1 API endpoints
    if (req.path.startsWith("/api/v1")) {
      const versionHeader = req.get("api-version");
      
      // If no version header or wrong version, return 400
      if (!versionHeader || versionHeader !== "1") {
        return next(createAppError(400, "Invalid or missing api-version header"));
      }
    }
